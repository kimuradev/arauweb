...
config.patternLab = {
  dir: './pattern-lab',
  public: './pattern-lab/public/styleguide/js'
};
...
config.html = {
  srcFiles: [
    './pattern-lab/source/templates/index.html'
  ],
  patternDir: './pattern-lab/public'
}

config.js = {
  srcFiles: [
    './bootstrap/javascripts/bootstrap.min.js',
    './pattern-lab/source/styleguide/js/*.js'
  ],
...
/**
 * Copy javascripts to pattern-lab js folder.
 */
...
/**
 * Copy index.html to pattern-lab public folder.
 */
gulp.task('pl:html', function () {
  gulp.src(config.html.srcFiles)
      .pipe(gulp.dest(config.html.patternDir));
});

/**
 * Gulp default task.
 */
gulp.task('pattern-lab', ['pl:svg', 'pl:js', 'pl:html', 'pl:server', 'pl:watch']);
